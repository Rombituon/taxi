...
#						frappe.publish_realtime(event='eval_js', message='cur_frm.set_value("last_note_time", {0}'.cur_time)
			...
			if int(d['last_note_time'].day) == cur_time.day:
				...
				if time_diff > int (d['re_notify_every']):
					msg_var = "Another Reminder Please follow up Trip Order # {0} as its delivery time is: {1} for customer: {2} and the time difference is {3} and notify before is {4}".format(d['name'], d['delivery_time'], d['customer'], time_diff, d['notify_before'])
					frappe.publish_realtime(event='msgprint', message=msg_var, user=frappe.session.user)
					frappe.db.set_value("Trip Order", d['name'], "last_note_time", cur_time)
#					frappe.publish_realtime(event='eval_js', message='refresh_field("last_note_time")')
				...
				if time_diff > int (d['re_notify_every']):
					msg_var = "Another Reminder For Next Day Please follow up Trip Order # {0} as its delivery time is: {1} for customer: {2} and the time difference is {3} and notify before is {4}".format(d['name'], d['delivery_time'], d['customer'], time_diff, d['notify_before'])
					frappe.publish_realtime(event='msgprint', message=msg_var, user=frappe.session.user)
					frappe.db.set_value("Trip Order", d['name'], "last_note_time", cur_time)
#					frappe.publish_realtime(event='eval_js', message='refresh_field("last_note_time")')
#		if d.notified == "Yes":
